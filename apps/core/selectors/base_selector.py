...
class BaseSelector(Generic[ModelType]):
    ...
    def get_active(self, qs: Optional[QuerySet[ModelType]] = None) -> QuerySet[ModelType]:
    ...
    def get_inactive(self, qs: Optional[QuerySet[ModelType]] = None) -> QuerySet[ModelType]:
        base_qs = self.get_all(qs)
        if hasattr(self.model, 'is_active') or hasattr(self.model(), 'is_active'):
            return base_qs.filter(is_active=False)
        return base_qs

    def get_by_id(self, record_id: Any, qs: Optional[QuerySet[ModelType]] = None) -> Optional[ModelType]:
        try:
    ...
    def filter(self, qs: Optional[QuerySet[ModelType]] = None, **kwargs: Any) -> QuerySet[ModelType]:
        return self.get_all(qs).filter(**kwargs)

    def search(self, search_fields: Sequence[str], query: str, qs: Optional[QuerySet[ModelType]] = None) -> QuerySet[ModelType]:
        """
    ...
    def order_by(self, *fields: str, qs: Optional[QuerySet[ModelType]] = None) -> QuerySet[ModelType]:
    ...
    def apply_optimizations(self, 
                            ...
                            qs: Optional[QuerySet[ModelType]] = None) -> QuerySet[ModelType]:
        """
